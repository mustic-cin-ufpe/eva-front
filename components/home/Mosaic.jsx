...
export default function Mosaic({ projectsRendered }) {
  ...
  function MosaicOrganizationWidth() {
    ...
    if (pageWidth <= 1500 && pageWidth > 640) {
      //3
      const firstColumn = projectsRendered.slice(
        0,
        lengthOfNewProjectsRendered / 3
      );
      const secondColumn = projectsRendered.slice(
        lengthOfNewProjectsRendered / 3,
        (2 * lengthOfNewProjectsRendered) / 3
      );
      const thirdColumn = projectsRendered.slice(
        (2 * lengthOfNewProjectsRendered) / 3,
        lengthOfNewProjectsRendered
      );
      ...
      const firstColumn = projectsRendered.slice(
        0,
        lengthOfNewProjectsRendered / 5
      );
      const secondColumn = projectsRendered.slice(
        lengthOfNewProjectsRendered / 5,
        (2 * lengthOfNewProjectsRendered) / 5
      );
      const thirdColumn = projectsRendered.slice(
        (2 * lengthOfNewProjectsRendered) / 5,
        (3 * lengthOfNewProjectsRendered) / 5
      );
      const forthColumn = projectsRendered.slice(
        (3 * lengthOfNewProjectsRendered) / 5,
        (4 * lengthOfNewProjectsRendered) / 5
      );
      const fifthColumn = projectsRendered.slice(
        (4 * lengthOfNewProjectsRendered) / 5,
        lengthOfNewProjectsRendered
      );
      setProjectsRenderedLength(projectsRendered.length);
      setArrayProjectsRendered([
        firstColumn,
        secondColumn,
        thirdColumn,
        forthColumn,
  ...
  function MosaicOrganizationAdding() {
    ...
    if (pageWidth <= 1500 && pageWidth > 640) {
      ...
      if (projectsRenderedLength == 0) {
        ...
        //adding projects
        const newProjectsAdded = projectsRendered.slice(
          projectsRenderedLength,
          lengthOfNewProjectsRendered
        );
        const newProjectsAddedLength = newProjectsAdded.length;
        arrayProjectsRendered[0]?.push(
          ...newProjectsAdded.slice(0, newProjectsAddedLength / 3)
        );
        arrayProjectsRendered[1]?.push(
          ...newProjectsAdded.slice(
            newProjectsAddedLength / 3,
            (2 * newProjectsAddedLength) / 3
          )
        );
        arrayProjectsRendered[2]?.push(
          ...newProjectsAdded.slice(
            (2 * newProjectsAddedLength) / 3,
            newProjectsAddedLength
          )
        );
        setProjectsRenderedLength(lengthOfNewProjectsRendered);
      }
      ...
      if (projectsRenderedLength == 0) {
        ...
        const newProjectsAddedLength = newProjectsAdded.length;
        arrayProjectsRendered[0]?.push(
          ...newProjectsAdded.slice(0, newProjectsAddedLength / 2)
        );
        arrayProjectsRendered[1]?.push(
          ...newProjectsAdded.slice(
            newProjectsAddedLength / 2,
            newProjectsAddedLength
          )
        );
        setProjectsRenderedLength(lengthOfNewProjectsRendered);
      ...
      if (projectsRenderedLength == 0) {
        ...
        const newProjectsAddedLength = newProjectsAdded.length;
        arrayProjectsRendered[0]?.push(
          ...newProjectsAdded.slice(0, newProjectsAddedLength / 5)
        );
        arrayProjectsRendered[1]?.push(
          ...newProjectsAdded.slice(
            newProjectsAddedLength / 5,
            (2 * newProjectsAddedLength) / 5
          )
        );
        arrayProjectsRendered[2]?.push(
          ...newProjectsAdded.slice(
            (2 * newProjectsAddedLength) / 5,
            (3 * newProjectsAddedLength) / 5
          )
        );
        arrayProjectsRendered[3]?.push(
          ...newProjectsAdded.slice(
            (3 * newProjectsAddedLength) / 5,
            (4 * newProjectsAddedLength) / 5
          )
        );
        arrayProjectsRendered[4]?.push(
          ...newProjectsAdded.slice(
            (4 * newProjectsAddedLength) / 5,
            newProjectsAddedLength
          )
